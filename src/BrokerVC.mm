...
{
    [super loadView];
    _recentFilterModel = new QSortFilterProxyModel;
    _recentFilterModel->setSourceModel(RecentModel::instance().peopleProxy());
...
- (void)placeTransfer:(id)sender
{
    ...
    auto call = RecentModel::instance().getActiveCall(originIdx);
    if (call != current) {
