...
{
    [super loadView];
    _recentFilterModel = new OnlyPersonProxyModel(RecentModel::instance()->peopleProxy());
...
- (void)placeTransfer:(id)sender
{
    ...
    if(m && !_recentFilterModel->index(0, 0, qIdx).isValid()){
        Call* c = CallModel::instance()->dialingCall();
        c->setPeerContactMethod(m);
        c << Call::Action::ACCEPT;

        [_smartView deselectAll:nil];
        [_smartView selectRowIndexes:[[NSIndexSet alloc] initWithIndex:0] byExtendingSelection:NO];
    }
}
