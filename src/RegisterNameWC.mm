...
- (IBAction)registerUsername:(id)sender
{
    ...
    self.isUserNameAvailable = AccountModel::instance().selectedAccount()->registerName(QString::fromNSString(self.password), QString::fromNSString(self.registeredName));
    ...
    registeredNameFound = QObject::connect(
                                           ...
                                           [=] ( const Account* account, NameDirectory::LookupStatus status,  const QString& address, const QString& name) {
                                               ...
                                               switch(status)
                                               {
                                                   ...
                                                   {
                                                       message = NSLocalizedString(@"The entered username is invalid. It must have at leat 3 characters and contains only lowercase alphanumeric characters.", @"Text shown to user when his username is invalid to be registered");
                                                       isAvailable = NO;
                                                       break;
                                                   }
