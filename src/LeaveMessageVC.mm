...
-(void) updateView {
    auto conversation = getConversationFromUid(conversationUid, *conversationModel);
    if (!conversation.has_value()) {
        [self show];
        return;
    }
    lrc::api::conversation::Info& conv = conversation.value();
    @autoreleasepool {
        auto& imgManip = reinterpret_cast<Interfaces::ImageManipulationDelegate&>(GlobalInstances::pixmapManipulator());
        QVariant photo = imgManip.conversationPhoto(conv, conversationModel->owner, QSize(120, 120), NO);
        [personPhoto setImage:QtMac::toNSImage(qvariant_cast<QPixmap>(photo))];
    }
    NSString *name = bestNameForConversation(conv, *conversationModel);
