...
@property (unsafe_unretained) IBOutlet NSTextField* infoField;
@property (unsafe_unretained) IBOutlet NSTextField* errorField;
...
- (void) setInfoMessageForAccount
{
    ...
    [infoMessage appendAttributedString:[[NSAttributedString alloc] initWithString:@"\n"]];
    [infoMessage appendAttributedString:[[NSAttributedString alloc] initWithString:NSLocalizedString(@"To proceed the migration, you must choose a password for your account. This password will be used to encrypt your master key. It will be required for adding new devices to your Ring account. If your not ready to choose a password, you may close Ring and resume the migration later.",@"Text shown to the user")]];
    [self.infoField setAttributedStringValue:infoMessage];
}
...
{
    [self.errorField setStringValue:NSLocalizedString(@"Failed to migrate your account. You can retry by pressing Ok or delete your account.",@"Error message shown to user when it is impossible to migrate account")];
    [self showError:nil];
}
