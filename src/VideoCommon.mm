...
+ (void) fillPixelBuffr:(CVPixelBufferRef &)pixelBuffer
    ...
    base = static_cast<uint8_t*>(CVPixelBufferGetBaseAddressOfPlane(pixelBuffer, 1));
    uint8_t* firstPlane = frame->data[1];
    uint8_t* secondPlane = frame->data[2];
    for(size_t i = 0; i < frame->height / 2 * bytesPerRowUV; i++){
        if (i % 2 == 0) {
            memcpy(base, firstPlane, 1);
            base = base + 1;
            firstPlane = firstPlane + 1;
        } else {
            memcpy(base, secondPlane, 1);
            base = base + 1;
            secondPlane = secondPlane + 1;
        }
    }
    CVPixelBufferUnlockBaseAddress(pixelBuffer, 0);
}
