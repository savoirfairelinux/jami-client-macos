...
std::map<lrc::api::ConferencableItem, lrc::api::ConferencableValue> values;
...
{
    values = convModel->getConferencableConversations(currentConversation, [searchField.stringValue UTF8String]);
    [self reloadView];
}
...
- (void) reloadView
{
    ...
    [contactsContainer setHidden: (contactsSize == 0 && !callsContainer.isHidden)];
      ...
      andCurrentConversation:(std::string)conversation
{
    ...
    values = convModel->getConferencableConversations(currentConversation, "");
    [self reloadView];
}
...
- (void)tableViewSelectionDidChange:(NSNotification *)notification
{
    ...
    std::pair<std::string, std::string> conversation;
    
    if (table == callsView) {
        conversation = values.find(lrc::api::ConferencableItem::CALL)->second[row].front();
        auto accountID = conversation.second;
        auto convID = conversation.first;
        ...
        conversation = values.find(lrc::api::ConferencableItem::CONTACT)->second[row].front();
        auto accountID = conversation.second;
        auto convID = conversation.first;
        auto convMod = convModel->owner.accountModel->getAccountInfo(accountID).conversationModel.get();
        auto conversationInfo = getConversationFromUid(convID, *convMod);
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    if (tableView == callsView && convModel != nullptr) {
        conversations = values.find(lrc::api::ConferencableItem::CALL)->second[row];
    } else if (tableView == contactsView && convModel != nullptr) {
        conversations = values.find(lrc::api::ConferencableItem::CONTACT)->second[row];
    }
    ...
    if(conversations.size() > 1) {
        ...
        NSArray* constraints = [NSLayoutConstraint
                                constraintsWithVisualFormat:@"H:|-(1)-[photoView(0)]"
                                options:0
    ...
    std::pair<std::string, std::string> conversation = conversations.front();
    auto accountID = conversation.second;
    auto convID = conversation.first;
    auto *convMod = convModel->owner.accountModel->getAccountInfo(accountID).conversationModel.get();
    auto *conversationInfo = &(*getConversationFromUid(convID, *convMod));
...
- (NSInteger)numberOfRowsInTableView:(NSTableView *)tableView
{
    ...
    if (tableView == contactsView) {
        return values.count(lrc::api::ConferencableItem::CONTACT) ? values.find(lrc::api::ConferencableItem::CONTACT)->second.size() : 0;
    } else if (tableView == callsView) {
        return values.count(lrc::api::ConferencableItem::CALL) ? values.find(lrc::api::ConferencableItem::CALL)->second.size() : 0;
    }
    return 0;
}
