...
- (void)tableViewSelectionDidChange:(NSNotification *)notification
{
    ...
    if (row == -1 || convModel == nil)
        return;
    auto conversations = callsView ? values[lrc::api::ConferenceableItem::CALL] : values[lrc::api::ConferenceableItem::CONTACT];
    if (conversations.size() < row) {
        return;
    }
    auto participants = conversations[row];
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    if (tableView == callsView && convModel != nullptr) {
        allConversations = values[lrc::api::ConferenceableItem::CALL];
    } else if (tableView == contactsView && convModel != nullptr) {
        allConversations = values[lrc::api::ConferenceableItem::CONTACT];
    }
...
- (NSInteger)numberOfRowsInTableView:(NSTableView *)tableView
{
    ...
    if (tableView == contactsView) {
        return values.count(lrc::api::ConferenceableItem::CONTACT) ? values[lrc::api::ConferenceableItem::CONTACT].size() : 0;
    } else if (tableView == callsView) {
        return values.count(lrc::api::ConferenceableItem::CALL) ? values[lrc::api::ConferenceableItem::CALL].size() : 0;
    }
    return 0;
}
