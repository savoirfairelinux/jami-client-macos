...
#import "views/RSVerticallyCenteredTextFieldCell.h"
...
-(void) awakeFromNib
{
    ...
    treeController = [[QNSTreeController alloc] initWithQModel:CategorizedContactModel::instance()];
...
- (NSCell *)outlineView:(NSOutlineView *)outlineView dataCellForTableColumn:(NSTableColumn *)tableColumn item:(id)item
{
    ...
    RSVerticallyCenteredTextFieldCell *returnCell = [tableColumn dataCell];
    if(!qIdx.isValid())
        return;

    if(!qIdx.parent().isValid()) {
        [returnCell setDrawsBackground:YES];
        [returnCell setBackgroundColor:[NSColor gridColor]];
    } else {
...
- (void)outlineView:(NSOutlineView *)olv willDisplayCell:(NSCell*)cell forTableColumn:(NSTableColumn *)tableColumn item:(id)item
{
    ...
    {
        if(!qIdx.parent().isValid()) {
            cell.title = CategorizedContactModel::instance()->data(qIdx, Qt::DisplayRole).toString().toNSString();
        } else {
            ContactMethod* meth = CategorizedContactModel::instance()->data(qIdx, (int)ContactMethod::Role::Object).value<ContactMethod*>();
            cell.title = CategorizedContactModel::instance()->data(qIdx, Qt::DisplayRole).toString().toNSString();
        }
    }
}
