...
-(void) updateView {
    ...
    if(QtMac::toNSImage(qvariant_cast<QPixmap>(photo))) {
        ...
        [photoView setImage: [QtMac::toNSImage(qvariant_cast<QPixmap>(photo)) roundCorners:300]];
               ...
               contextInfo:(void*) contextInfo
{
    if (auto outputImage = [picker outputImage]) {
        [photoView setBordered:NO];
        [photoView setImage: [outputImage roundCorners:300]];
        [addProfilePhotoImage setHidden:YES];
...
- (IBAction)showBanned: (NSButton *)sender {
    ...
    if(height == 150) {
        frame.size.height =  frame.size.height - 150 - 20 ;
    } else {
        frame.size.height =  frame.size.height + 150 + 20 ;
    }
    ...
    self.advancedButtonMarginConstraint.constant = (height== 2) ? 20 : 0;
