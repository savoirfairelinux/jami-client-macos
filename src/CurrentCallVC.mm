...
@interface CurrentCallVC () <NSPopoverDelegate, ContactLinkedDelegate> {
    ...
    const lrc::api::call::Info *currentCall_;
}
...
{
    if (not callModel_->hasCall(call->id)){
        currentCall_ = nil;
        return;
    }
    currentCall_ = call;
...
-(id) initWithNibName:(NSNibName)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil account:(lrc::api::account::Info *)currentAccount
{
    [self setSelectedAccount:currentAccount];
...
-(void) updateCall:(BOOL) firstRun withId: (const std::string *) callId
{
    if (not callModel_->hasCall(*callId)) {
        return;
    }
    NSLog(@"\n status %@ \n",@(lrc::api::call::to_string(callModel_->getCall(*callId).status).c_str()));
    [personLabel setStringValue:@(currentCall_->peer.c_str())];
   // [timeSpentLabel setStringValue:callIdx.data((int)Call::Role::Length).toString().toNSString()];
    [stateLabel setStringValue:@(to_string(currentCall_->status).c_str())];
...
//    auto contvactmethod = qvariant_cast<Call*>(callIdx.data(static_cast<int>(Call::Role::Object)))->peerContactMethod();
...
//    [self.addContactButton setHidden:!shouldShow];
    ...
    switch (currentCall_->status) {
        ...
        case Status::INCOMING_RINGING:
            ...
            [outgoingPanel setHidden:YES];
            [self setupIncoming:&currentCall_->id];
            break;
...
-(void) connectVideoSignals
{
    ...
    if (callModel_->hasCall(currentCall_->id)) {
        if (auto renderer = callModel_->getRenderer(currentCall_->id)) {
            QObject::disconnect(self.videoStarted);
            [self connectVideoRenderer: renderer];
        }
    }
...
-(void) animateIn
{
    const std::string *callId = &currentCall_->id;
    ...
    [CATransaction setCompletionBlock:^{
        ...
        self.messageConnection = QObject::connect(conversationModel_,
...
- (IBAction)hangUp:(id)sender {
    callModel_->hangUp(currentCall_->id);
}

- (IBAction)accept:(id)sender {
    callModel_->accept(currentCall_->id);
}

- (IBAction)toggleRecording:(id)sender {
    callModel_->toggleAudioRecord(currentCall_->id);
}

- (IBAction)toggleHold:(id)sender {
    callModel_->togglePause(currentCall_->id);
}
...
{
    callModel_->toggleMedia(currentCall_->id, lrc::api::NewCallModel::Media::AUDIO);
}

- (IBAction)muteVideo:(id)sender
{
    callModel_->toggleMedia(currentCall_->id, lrc::api::NewCallModel::Media::VIDEO);
}
