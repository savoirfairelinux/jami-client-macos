...
-(void)updateConferenceOverlays:(QString)callID {
    ...
    switch (call.status) {
        ...
        case Status::TIMEOUT:
            ...
            if (callID == callUid_ && callModel->hasCall(confUid_)) {
                if (subcalls.isEmpty()) {
                    auto activeCalls = [appDelegate getActiveCalls];
                    if(activeCalls.isEmpty()) {
                        return;
                    }
                    auto newC = callModel->getCall(activeCalls[0]);
                    auto conv = getConversationFromURI(newC.peerUri, *accountInfo_->conversationModel);
                    [self.delegate chooseConversation: *conv model:accountInfo_->conversationModel.get()];
                    [self updateCall];
                    return;
                }
                for(auto subcall: subcalls) {
                    qDebug() << "***cubcall" << subcall;
                    if(subcall != callID) {
                        auto newCall = callModel->getCall(subcall);
