...
BOOL displayGreedLayoputButton = false;
...
-(void)removeConferenceLayout {
    ...
    displayGreedLayoputButton = false;
}
...
-(void)updateConference
{
    ...
    displayGreedLayoputButton = call.layout != lrc::api::call::Layout::GRID;
    ...
    self.mozaicLayoutButton.hidden = !displayGreedLayoputButton;
    self.mozaicLayoutButton.enabled = displayGreedLayoputButton;
}
...
- (IBAction)setGreedLayout:(id)sender {
    if (accountInfo_ == nil)
        return;
    auto* callModel = accountInfo_->callModel.get();
