...
NSLock* distantFrameLk;
NSLock* previewFrameLk;
...
-(void) renderer: (Video::Renderer*)renderer renderFrameForPreviewView:(CallMTKView*) view
{
    @autoreleasepool {
        [previewFrameLk lock];
        auto currentFrame = renderer->currentAVFrame();
        auto frame = (const AVFrame *) currentFrame;
        if(!frame || !frame->width || !frame->height) {
            [previewFrameLk unlock];
            return;
        }
