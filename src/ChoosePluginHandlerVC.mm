...
-(IBAction)toggleHandler:(id)sender
{
    ...
    pluginModel->toggleCallMediaHandler(availableHandlers[row], currentCall, false);
}
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    NSTextField* pluginHandlerName = [result viewWithTag:HANDLER_NAME_TAG];
    NSImageView* iconView = [result viewWithTag:ICON_TAG];
    NSSwitch* handlerStatusView = [result viewWithTag:HANDLER_STATUS_TAG];
    ...
    if (handlerType == FROM_CALL) {
        ...
        activeHandlers = pluginModel->getChatHandlerStatus(currentAccount, currentConvID);
        handlerDetails = pluginModel->getCallMediaHandlerDetails(handler);
    }
