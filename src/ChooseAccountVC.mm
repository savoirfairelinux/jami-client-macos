...
-(const lrc::api::account::Info&) getSavedAccount
{
    auto& account = accMdl_->getAccountInfo(std::string([[accountManager getSavedAccountId] UTF8String]));
    if(account.profileInfo.type == lrc::api::profile::Type::INVALID){
        try {
            auto& accountId = accMdl_->getAccountList().at(0);
            [accountManager saveAccountWithId:@(accountId.c_str())];
            return accMdl_->getAccountInfo(accMdl_->getAccountList().at(0));
        } catch (std::out_of_range& e) { // Is thrown if account model has no account. We then return an invalid account
            return account;
        }
    }
    return accMdl_->getAccountInfo(std::string([[accountManager getSavedAccountId] UTF8String]));
}
...
-(void) setPopUpButtonSelection {
    ...
    auto& account = [self getSavedAccount];
    if(account.profileInfo.type == lrc::api::profile::Type::INVALID){
        return;
    }
...
- (IBAction)itemChanged:(id)sender {
    ...
    [accountManager saveAccountWithId:@(account.id.c_str())];
}
