...
{
    this->addressBookObserver = [[NSNotificationCenter defaultCenter] addObserverForName:kABDatabaseChangedNotification
                                     object:nil
                                     queue:[NSOperationQueue mainQueue]
                                     usingBlock:^(NSNotification *note) {
                                         for (NSString* r in note.userInfo[kABInsertedRecords]) {
                                         ...
                                         for (NSString* r in note.userInfo[kABUpdatedRecords]) {
                                             NSLog(@"Updated person: %@", r);
                                         }

                                         for (NSString* r in note.userInfo[kABDeletedRecords]) {
...
{
    [[NSNotificationCenter defaultCenter] removeObserver:this->addressBookObserver];
}
