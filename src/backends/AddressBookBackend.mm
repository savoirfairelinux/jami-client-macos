...
bool AddressBookBackend::load()
{
    ...
    {
        Person* person = new Person(QByteArray::fromNSData(p.vCardRepresentation),
        ...
        if([person->formattedName().toNSString() isEqualToString:@""]   &&
           ...
           [person->firstName().toNSString() isEqualToString:@""]) {
            qDebug() << QByteArray::fromNSData(p.vCardRepresentation);
            continue;
        }


        qDebug() << "Adding: " << person->formattedName() << " " << person->secondName() << " " << person->firstName();
