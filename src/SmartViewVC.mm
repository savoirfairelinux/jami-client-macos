...
- (void) startConversationFromURI:(URI&) uri
{
...
- (void) processSearchFieldInput
{
    ...
    if (numberEntered && numberEntered.length > 0) {
        ...
        if (uri.protocolHint() == URI::ProtocolHint::RING_USERNAME) {
            ...
            usernameLookupConnection = QObject::connect(&NameDirectory::instance(),
                                                     ...
                                                     [self, usernameToLookup] (const Account* account, NameDirectory::LookupStatus status, const QString& address, const QString& name) {
