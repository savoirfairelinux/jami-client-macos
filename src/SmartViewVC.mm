...
- (void)awakeFromNib
{
    ...
    //[searchField setBackgroundColor:[NSColor colorWithCalibratedRed:0.949 green:0.949 blue:0.949 alpha:0.9]];
}
...
- (void)outlineViewSelectionDidChange:(NSNotification *)notification
{
    ...
    if([[treeController selectedNodes] count] > 0) {
        QModelIndex srcIdx = RecentModel::instance()->peopleProxy()->mapToSource(qIdx);
        if(RecentModel::instance()->hasActiveCall(srcIdx)) {
            auto selected = RecentModel::instance()->getActiveCall(srcIdx);
...
- (NSView *)outlineView:(NSOutlineView *)outlineView viewForTableColumn:(NSTableColumn *)tableColumn item:(id)item
{
    ...
    QModelIndex srcIdx = RecentModel::instance()->peopleProxy()->mapToSource(qIdx);
    BOOL ongoing = RecentModel::instance()->hasActiveCall(srcIdx);

    [[result viewWithTag:CALL_BUTTON_TAG] setHidden:ongoing];
    [[result viewWithTag:TXT_BUTTON_TAG] setHidden:ongoing];
    NSTextField* details = [result viewWithTag:DETAILS_TAG];

    if (ongoing) {
        auto call = RecentModel::instance()->getActiveCall(srcIdx);
        [details setStringValue:call->toHumanStateName().toNSString()];
