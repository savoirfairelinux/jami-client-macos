...
- (void) addToContact
{
    ...
    auto contactmethod = RecentModel::instance().extractContactMethodsFromIndex(originIdx);
...
- (void) onAddShortcut
{
    auto qIdx = [treeController toQIdx:[treeController selectedNodes][0]];
    auto originIdx = RecentModel::instance().peopleProxy()->mapToSource(qIdx);
    auto contactmethods = RecentModel::instance().extractContactMethodsFromIndex(originIdx);
