...
- (void)loadAccount:(Account *)account
{
    ...
    if([account->alias().toNSString() isEqualToString:@"IP2IP"]) {
        ...
        NSLog(@"boxingAccount %f, %f", boxingAccount.frame.origin.x, boxingAccount.frame.origin.y);
        NSLog(@"boxingCommon %f, %f", boxingCommon.frame.origin.x, boxingCommon.frame.origin.y);

        [boxingCommon setFrame:boxingAccount.frame];

        [upnpButton setFrame:boxingAccount.frame];

        [boxingCommon setNeedsLayout:YES];
