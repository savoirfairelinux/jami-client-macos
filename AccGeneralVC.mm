...
- (IBAction)toggleUpnp:(NSButton *)sender {
    //FIXME: not integrated in libringclient
}
...
- (void)loadAccount:(Account *)account
{
    ...
    if([account->alias().toNSString() isEqualToString:@"IP2IP"]) {
        ...
        QModelIndex qIdx = account->protocolModel()->selectionModel()->currentIndex();
        [self.typeLabel setStringValue:account->protocolModel()->data(qIdx, Qt::DisplayRole).toString().toNSString()];
...
//  [self.upnpButton setState:privateAccount-]
//  [self.userAgentButton setState:privateAccount-]
